...
let Zoroark=new Pokemon('Zoroark',14,8,100,0.5);
let Ectoplasma=new Pokemon('Ectoplasma',18,4,70,0.8);

while(Zoroark.hp >0 && Ectoplasma.hp>0){
    Zoroark.attackPokemon(Ectoplasma)
    if(Ectoplasma.hp<=0){
        console.log(Ectoplasma.name+" is dead !");
        break;
    }
    Ectoplasma.attackPokemon(Zoroark)
    if(Zoroark.hp<=0){
        console.log(Zoroark.name+" is Dead !");
        break
    }
}